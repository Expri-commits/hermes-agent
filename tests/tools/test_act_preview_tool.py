"""Tests for the GUI-surface ``act_preview`` tool."""

import json

from tools import act_preview_tool as ap
...
def test_lives_in_the_gui_surface_toolset(monkeypatch):
    ...
    entry = registry.get_entry("act_preview")
...
def test_requires_callback():
    ...
    result = json.loads(ap.act_preview_tool(action="elements", callback=None))
    assert "desktop" in result["error"]


def test_rejects_an_unknown_action():
    result = json.loads(ap.act_preview_tool(action="teleport", callback=lambda _p: "{}"))
...
def test_interaction_verbs_need_a_target():
    ...
    for verb in ("click", "type", "press"):
        result = json.loads(ap.act_preview_tool(action=verb, text="x", key="Enter", callback=lambda _p: "{}"))
...
def test_type_needs_text_and_press_needs_a_key():
    ...
    assert "text" in json.loads(ap.act_preview_tool(action="type", ref="@e1", callback=cb))["error"]
    assert "key" in json.loads(ap.act_preview_tool(action="press", ref="@e1", callback=cb))["error"]
    assert calls == []
...
def test_typing_an_empty_string_is_allowed():
    ...
    seen = {}
    ap.act_preview_tool(action="type", ref="@e1", text="", callback=lambda p: seen.update(p) or "{}")
...
def test_scroll_needs_no_target_and_validates_its_destination():
    ...
    ap.act_preview_tool(action="scroll", callback=cb)
    assert seen == {"action": "scroll"}

    result = json.loads(ap.act_preview_tool(action="scroll", to="sideways", callback=cb))
...
def test_numeric_arguments_are_validated():
    result = json.loads(ap.act_preview_tool(action="scroll", amount="lots", callback=lambda _p: "{}"))
    assert "integers" in result["error"]


def test_empty_answer_means_nothing_open():
    result = json.loads(ap.act_preview_tool(action="elements", callback=lambda _p: ""))
...
def test_passes_the_renderer_answer_through():
    ...
    result = json.loads(ap.act_preview_tool(action="click", ref="@e2", callback=lambda _p: json.dumps(payload)))

    assert result == payload


def test_wraps_non_json_text():
    result = json.loads(ap.act_preview_tool(action="elements", callback=lambda _p: "plain words"))
...
def test_callback_failure_is_reported():
    ...
    result = json.loads(ap.act_preview_tool(action="elements", callback=_boom))
