...
ACTIONS = ("elements", "click", "type", "scroll", "press", "back", "forward", "reload")
...
NEEDS_TARGET = ("click", "type", "press")


def act_preview_tool(
    action: str = "",
    ...
    if callback is None:
        return tool_error("act_preview is only available in the Hermes desktop app.")
...
ACT_PREVIEW_SCHEMA = {
    ...
    "description": (
        ...
        "navigation; when told they are stale, call elements again. Actions: "
        "'elements' (inventory), 'click', 'type' (set a field's text; "
        "submit=true also presses Enter and submits the form), 'scroll' "
        "(the page, or a ref'd scrollable), 'press' (a named key), and "
        "'back'/'forward'/'reload' for history. Use read_preview when you only "
...
registry.register(
    ...
    handler=lambda args, **kw: act_preview_tool(
