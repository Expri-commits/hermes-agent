...
| `act_preview` | Interact with the page open in the in-app browser: `elements` inventories what's clickable and typable (each with an `@e1`-style ref, role, label, and value), then `click`, `type`, `scroll`, and `press` act on a ref, and `back`/`forward`/`reload` drive the pane's history. Every action answers with the live URL and a refreshed inventory. | — |
